/* eslint-disable */
import Long from "long";
import _m0 from "protobufjs/minimal";

export const protobufPackage = "testingpb";

/**
 * To compile the proto, run:
 *      protoc --go_out=plugins=grpc:$GOPATH/src *.proto
 */

export interface TestPayload {
  msgBody: Uint8Array;
}

const baseTestPayload: object = {};

export const TestPayload = {
  encode(
    message: TestPayload,
    writer: _m0.Writer = _m0.Writer.create()
  ): _m0.Writer {
    if (message.msgBody.length !== 0) {
      writer.uint32(10).bytes(message.msgBody);
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): TestPayload {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseTestPayload } as TestPayload;
    message.msgBody = new Uint8Array();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
          message.msgBody = reader.bytes();
          break;
        default:
          reader.skipType(tag & 7);
          break;
      }
    }
    return message;
  },

  fromJSON(object: any): TestPayload {
    const message = { ...baseTestPayload } as TestPayload;
    message.msgBody = new Uint8Array();
    if (object.msgBody !== undefined && object.msgBody !== null) {
      message.msgBody = bytesFromBase64(object.msgBody);
    }
    return message;
  },

  toJSON(message: TestPayload): unknown {
    const obj: any = {};
    message.msgBody !== undefined &&
      (obj.msgBody = base64FromBytes(
        message.msgBody !== undefined ? message.msgBody : new Uint8Array()
      ));
    return obj;
  },

  fromPartial(object: DeepPartial<TestPayload>): TestPayload {
    const message = { ...baseTestPayload } as TestPayload;
    if (object.msgBody !== undefined && object.msgBody !== null) {
      message.msgBody = object.msgBody;
    } else {
      message.msgBody = new Uint8Array();
    }
    return message;
  },
};

declare var self: any | undefined;
declare var window: any | undefined;
var globalThis: any = (() => {
  if (typeof globalThis !== "undefined") return globalThis;
  if (typeof self !== "undefined") return self;
  if (typeof window !== "undefined") return window;
  if (typeof global !== "undefined") return global;
  throw "Unable to locate global object";
})();

const atob: (b64: string) => string =
  globalThis.atob ||
  ((b64) => globalThis.Buffer.from(b64, "base64").toString("binary"));
function bytesFromBase64(b64: string): Uint8Array {
  const bin = atob(b64);
  const arr = new Uint8Array(bin.length);
  for (let i = 0; i < bin.length; ++i) {
    arr[i] = bin.charCodeAt(i);
  }
  return arr;
}

const btoa: (bin: string) => string =
  globalThis.btoa ||
  ((bin) => globalThis.Buffer.from(bin, "binary").toString("base64"));
function base64FromBytes(arr: Uint8Array): string {
  const bin: string[] = [];
  for (const byte of arr) {
    bin.push(String.fromCharCode(byte));
  }
  return btoa(bin.join(""));
}

type Builtin =
  | Date
  | Function
  | Uint8Array
  | string
  | number
  | boolean
  | undefined;
export type DeepPartial<T> = T extends Builtin
  ? T
  : T extends Array<infer U>
  ? Array<DeepPartial<U>>
  : T extends ReadonlyArray<infer U>
  ? ReadonlyArray<DeepPartial<U>>
  : T extends {}
  ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

if (_m0.util.Long !== Long) {
  _m0.util.Long = Long as any;
  _m0.configure();
}
